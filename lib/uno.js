...
function unoGame(config){
    ...
    var defaultSendMessage = async function(message, text, isPrivate){
        if (isPrivate){
            await message.reply({content: text, ephemeral: true});
        }
    ...
    var drawCards = async function(message, game, playerName, count){
        ...
        try {
            var url = 'https://deckofcardsapi.com/api/deck/' + game.deckId + '/draw/?count=' + count;
            var cardRequest = await request({ uri: url, json: true });

            if (!cardRequest.cards)
            {
                console.error('An error occurred while trying to load the url "' + url + '".');
                console.error(cardRequest);
                ...
                if (!game.deckId)
                {
                    ...
                    if (!game.deckId)
                    {
                        console.error('Deck could not be refreshed');
                    }
                }
                
                await sendMessage(message, 'Sorry, I\'m having trouble loading the deck. Please try again later');
                return;
            }
            ...
            if (cardRequest.remaining <= 10){
                ...
                await request({ uri: 'https://deckofcardsapi.com/api/deck/' + game.deckId + '/shuffle/', json: true });
    ...
    var endGame = async function(message, game){
        ...
        if (currentScores[0].Score >= 500){
            ...
            await sendMessage(message, {
                "text": '@' + game.player1 + ', type `/uno start` to begin a new hand.',
                "link_names": true
            });
    ...
    var getNewDeck = async function(game){
        var deckRequest = await request({ uri: 'https://deckofcardsapi.com/api/deck/new/shuffle/?deck_count=2', json: true });
    ...
    this.beginGame = async function(message, game){
        ...
        //Shuffle current turn order
        var oldTurnOrder = game.turnOrder;
        var newTurnOrder = shuffle(oldTurnOrder);
        console.log('Turn order shuffled');
        console.log('Original order:');
        console.log(oldTurnOrder);
        ...
        try {
            ...
            var startingCardRequest = await request({ uri: 'https://deckofcardsapi.com/api/deck/' + game.deckId + '/draw/?count=1', json: true });
    ...
    this.beginTurnInteractive = async function(message, game){
        ...
        if (!game){
          /*TODO: Prompt for user to create/start/join/etc the game */
            return;
        }
